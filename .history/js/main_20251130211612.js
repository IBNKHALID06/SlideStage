...
els.recordStart.addEventListener('click', async () => {
  try {
    ...
    // Priority: MP4 (native) -> WebM (H.264) -> WebM (Generic)
    // We avoid specifying audio codecs (like opus) to let the browser pick the best compatible one.
    ...
    if (!MediaRecorder.isTypeSupported(mimeType)) {
      // Fallback to generic WebM (browser chooses codecs)
      mimeType = 'video/webm';
    }
