export class WebcamOverlay {
  constructor(boxEl, videoEl, settings) {
    ...
    const pos = this.settings.get('webcam.pos');
    if (pos) {
    ...
    const size = this.settings.get('webcam.size');
    if (size) {
  ...
  _bindDrag() {
    ...
    const onMouseUp = () => {
      ...
      const rect = this.boxEl.getBoundingClientRect();
      this.settings.set('webcam.pos', { left: rect.left, top: rect.top });
    };
  ...
  _bindResize() {
    ...
    const onMouseUp = () => {
      ...
      resizing = false;
      const rect = this.boxEl.getBoundingClientRect();
      this.settings.set('webcam.size', { w: rect.width, h: rect.height });
    };
