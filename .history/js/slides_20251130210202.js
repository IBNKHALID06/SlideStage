export class SlideViewer {
  ...
  async render() {
    ...
    try {
      // Ensure canvas is visible (in case fallback hid it previously)
      this.canvas.style.display = 'block';
      // Remove any fallback iframe if it exists
      const container = this.canvas.parentElement;
      const iframe = container?.querySelector('iframe[data-fallback-pdf]');
  ...
  async next() {
    console.log('Next slide requested. Current:', this.pageNum, 'Total:', this.total);
    if (!this.pdf && !this.useFallback) {
  ...
  async prev() {
    console.log('Prev slide requested. Current:', this.pageNum);
    if (!this.pdf && !this.useFallback) {
      console.warn('No PDF loaded');
      return;
    }
