...
els.recordStart.addEventListener('click', async () => {
  try {
    ...
    // Priority: MP4 (native) -> WebM (H.264) -> WebM (VP9) -> WebM (VP8) -> Default
    let mimeType = 'video/mp4';
    if (!MediaRecorder.isTypeSupported(mimeType)) {
      // Try H.264 in WebM (often supported and very compatible)
    ...
    recorder.onstop = () => {
      ...
      if (mimeType.includes('mp4')) {
        ...
        // It's a WebM container but H.264 video. 
        // Some players prefer .mp4 extension even if container is webm-ish, 
        // but strictly it's .webm. However, for "normal player" compatibility, 
        // .webm is usually fine if codec is H.264.
        ext = 'webm'; 
      }
      ...
      els.recordStatus.textContent = `Recording saved as ${ext.toUpperCase()} (${mimeType}).`;
